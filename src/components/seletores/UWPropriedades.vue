...
const filtersSearch = [
    { field: 'identificacao', matchMode: 'contains', tipoField: 'text', fieldFilter: 'nomeLocal', labelFilter: 'Nome' }
];
...
const filterSearchAtivo = {
    ...
    matchMode: 'contains',
    tipo: 'text',
    fieldFilter: 'identificacao',
    labelFilter: 'Identificação'
};
...
const getLista = async (forceId) => {
    try {
        ...
        const data = await Service.getPropriedades(props.parceiroLocalId, lazyParams.value);
...
const labelSelector = computed(() => {
    if (propriedade.value) {
        return `${propriedade.value.identificacao}`;
    }
    return '';
});
                    ...
                    <div style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-wrap: wrap">
                        ...
                        >
                            <div class="flex flex-column align-items-start">
                                <span>{{ slotProps.option.identificacao }}</span>
                            </div>
                        </div>
                        <div
