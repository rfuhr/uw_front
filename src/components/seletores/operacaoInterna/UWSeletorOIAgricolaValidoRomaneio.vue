...
const getLista = async () => {
    try {
        await montarFiltros();
        const data = await Service.getSeletorByOperacaoInternaValidadoParaRomaneio(lazyParams.value, props.itemId || -1, props.grupoOperacaoAgricolaId || -1);
...
watch([() => props.itemId, () => props.grupoOperacaoAgricolaId], async () => {
    await getLista();
});
