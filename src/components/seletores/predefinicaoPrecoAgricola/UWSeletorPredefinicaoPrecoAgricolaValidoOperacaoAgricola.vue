...
const getLista = async () => {
    try {
        await montarFiltros();
        const data = await Service.getSeletorByValidadoParaOperacaoAgricola(lazyParams.value, props.itemId || -1, props.tipoPrecoAgricolaId || -1);
...
watch([() => props.itemId, () => props.tipoPrecoAgricolaId], async () => {
    await getLista();
});
