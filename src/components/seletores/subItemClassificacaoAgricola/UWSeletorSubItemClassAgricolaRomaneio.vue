...
const props = defineProps({
    ...
    placeholder: {
        type: String,
        default: 'Selecione o sub. item de classificação'
    },
    required: {
        type: Boolean,
        default: false
...
const getLista = async () => {
    try {
        await montarFiltros();
        const data = await Service.buscarParaRomaneio(lazyParams.value, props.itemId || -1, props.itemClassificacaoAgricolaId || -1, formatDate(props.dataRomaneio, 'yyyy-MM-dd') || new Date());
