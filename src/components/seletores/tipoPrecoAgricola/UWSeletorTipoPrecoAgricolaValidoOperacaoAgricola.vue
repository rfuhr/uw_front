...
const props = defineProps({
    ...
    placeholder: {
        type: String,
        default: 'Selecione a operação interna'
    },
    required: {
        type: Boolean,
        default: false
...
const getLista = async () => {
    try {
        await montarFiltros();
        const data = await Service.getSeletorByValidadoParaOperacaoAgricola(lazyParams.value, props.itemId || -1, props.grupoOperacaoAgricolaId || -1, props.operacaoInternaId || -1);
...
watch([() => props.itemId, () => props.grupoOperacaoAgricolaId, () => props.operacaoInternaId], async () => {
    await getLista();
});
