...
import UWContexto from '../components/layout/UWContexto.vue';
import { AuthService, ModuloService, ContextoService } from '../service';
...
const { onMenuToggle, onTopbarMenuToggle } = useLayout();
const router  = useRouter();
const contextoStore = useContexto()
const { logout, infoUser } = useAuth();
const modulos = ref([])
const showDialogContexto = ref(false)
const empresaTrocarSelecionada = ref({empresaNome: '', empresaId: 0})
const empresaFilialTrocarSelecionada = ref({empresaFilialId: 0, empresaFilialNome: ''})
...
const loadModulos = async () => {
    try {
        modulos.value = await ModuloService.getAll();
    } catch {
        modulos.value = []
    }
}

const mudarModulo = async (modulo) => {
    const contextoAlterar = _.cloneDeep(contextoStore.contexto)
	contextoAlterar.moduloId = modulo.id
	try {
   
        const data = await ContextoService.alterarContexto(contextoAlterar)
		contextoStore.setContexto(data);
        router.push({ path: `/${modulo.pathBase}` });
        toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Novo módulo selecionado', life: 5000});
    } catch (err) {
        toast.add({ severity: 'error', summary: 'Falha', detail: 'Não foi possível selecionar novo módulo', life: 5000});                   

    } 
}

const mudarContexto = async () => {
    showDialogContexto.value = true;
}

const trocarContexto = async () => {
    submitted.value = true;
	if (empresaTrocarSelecionada.value.empresaId && empresaFilialTrocarSelecionada.value.empresaFilialId) {
        const contextoAlterar = _.cloneDeep(contextoStore.contexto)
        contextoAlterar.empresaId = empresaTrocarSelecionada.value.empresaId;
		contextoAlterar.empresaFilialId = empresaFilialTrocarSelecionada.value.empresaFilialId;
        try {
            const data = await ContextoService.alterarContexto(contextoAlterar)
            contextoStore.setContexto(data)
            router.replace({ path: `/${data.moduloPathBase}` })
            showDialogContexto.value = false;
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Contexto alterado com sucesso', life: 5000});
        } catch {
			toast.add({ severity: 'error', summary: 'Falha', detail: 'Não foi possível alterar o contexto', life: 5000}); 
		}
    }
}
    ...
    <div class="layout-topbar">
        <div class="layout-topbar-start" style="background-color: white;">
            <router-link class="layout-topbar-logo" to="/">
                ...
                <!-- <div width="41" height="32" viewBox="0 0 41 32" fill="none" xmlns="http://www.w3.org/2000/svg" class="layout-topbar-logo-slim"> -->
                    <!-- <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        ...
                        fill="var(--topbar-item-text-color)"
                    /> -->
                    <img src="/layout/images/logo/logo-top.png" alt="" width="78" height="48" viewBox="0 0 41 32" fill="none" class="layout-topbar-logo-slim"/>
                <!-- </svg> -->
            </router-link>
                ...
                <ul class="layout-topbar-items">
                     <li>
                        ...
                        <div class="hidden">
                            <div class="flex flex-wrap">
                                <div class="w-5 flex flex-column align-items-center p-3" v-for="modulo in modulos" :key="modulo.id">
                                    <Button class="mb-2" rounded  outlined :icon="`${modulo.icone}`" @click="mudarModulo(modulo)"></Button>
                    ...
                    <li>
                        ...
                        >
                            ...
                            <Dialog v-model:visible="showDialogContexto" :style="{width: '60%'}" header="Mudar Contexto" :modal="true" class="p-fluid">
		                        <Fieldset legend="Contexto Atual" class="content-alt-bgcolor">
			                        <div class="field">
				                        <label for="empresaTrocar" class="mb-3">Empresa</label>
				                        <InputText type="text" v-model="contextoStore.getNomeEmpresaContexto" disabled />
			                        </div>
			                        <div class="field">
				                        <label for="empresaFilialTrocar" class="mb-3">Filial</label>
				                        <InputText type="text" v-model="contextoStore.getNomeEmpresaFilialContexto" disabled />
			                        </div>		
		                        </Fieldset> 
		                         <Fieldset legend="Novo Contexto" class="content-bgcolor">
			                        <div class="field">
				                        <label for="empresaTrocar" class="mb-3">Empresa</label>
				                        <Dropdown id="empresaTrocar" v-model="empresaTrocarSelecionada" :options="contextoStore.getListaEmpresasOrganograma" 
					                        optionLabel="empresaNome" 
					                        placeholder="Selecione uma empresa" 
					                        @change="onChangeEmpresa"
					                        required="true"
					                        :class="{'p-invalid': submitted && !empresaTrocarSelecionada.empresaId}"/>
				                        <small class="p-invalid" style="color: red" v-if="submitted && !empresaTrocarSelecionada.empresaId">Empresa deve ser informada.</small>
			                        </div>
			                        <div class="field">
				                        <label for="empresaFilialTrocar" class="mb-3">Filial</label>
				                        <Dropdown id="empresaFilialTrocar" v-model="empresaFilialTrocarSelecionada" :options="contextoStore.getListaEmpresasFiliaisOrganograma(empresaTrocarSelecionada.empresaId)" optionLabel="empresaFilialNome" placeholder="Selecione uma filial"
					                        required="true"
					                        :class="{'p-invalid': submitted && !empresaFilialTrocarSelecionada.empresaFilialId}"/>
				                        <small class="p-invalid" style="color: red" v-if="submitted && !empresaFilialTrocarSelecionada.empresaFilialId">Filial deve ser informada.</small>
			                        </div>		
		                        </Fieldset> 
		                        <template #footer>
			                        <Button label="Cancelar" icon="pi pi-times" class="p-button-text p-button-secondary" @click="hideDialog"/>
			                        <Button label="Salvar" icon="pi pi-check" class="p-button-text" @click="trocarContexto" />
		                        </template>		
	                        </Dialog>
                    </li>      
                    <li>
                        ...
                        <div class="hidden">
                            <ul class="list-none p-0 m-0">
                                <li>
                                    <span class="p-ripple cursor-pointer flex align-items-center py-2 hover:surface-hover transition-duration-150 transition-all px-3 py-2" v-ripple>
                                        <span><strong>Usuário: </strong>{{ infoUser.value.nome }}</span>
                                    </span>
                                </li>
                                <li>
                                    <Divider />
                                ...
                                <li>
                                    <a @click="realizarLogout" class="p-ripple cursor-pointer flex align-items-center py-2 hover:surface-hover transition-duration-150 transition-all px-3 py-2" v-ripple>
