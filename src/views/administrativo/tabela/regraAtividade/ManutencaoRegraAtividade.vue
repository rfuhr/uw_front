...
import { RegraAtividadeService as Service } from '@/service';
...
const criarRegistro = () => {
    Service.create(formData)
        .then(() => {
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Regra de atividade criada com sucesso', life: 5000 });
            emit('closeDialog');
        })
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Não foi possível criar a regra de atividade.', life: 5000 });
        });
};
...
const alterarRegistro = () => {
    ...
    Service.update(formData)
        .then(() => {
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Regra de atividade alterada com sucesso', life: 5000 });
            emit('closeDialog');
        })
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Não foi possível alterar a regra de atividade.', life: 5000 });
        });
};
...
<template>
    <Dialog v-model:visible="showDialogComputed" :style="{ width: '40%' }" :header="mode === 'create' ? 'Nova Regra de Atividade' : 'Alterar Regra de Atividade'" :modal="true" :closable="false" @show="showModal">
        <UWForm :schema="schema" :values="formData" ref="formRegraAtividade" @doCancel="hideDialog" @doSubmit="salvarRegistro">
