...
import { OrdemCalculoAgricolaService as Service, ItemService, TiposService, TipoCalculoAgricolaService } from '@/service';
...
const criarRegistro = () => {
    Service.create(formData)
        .then(() => {
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Ordem de cálculo criada com sucesso', life: 5000 });
            emit('closeDialog');
        })
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Não foi possível criar a ordem de cálculo.', life: 5000 });
        });
};
...
const alterarRegistro = () => {
    ...
    Service.update(formData)
        .then(() => {
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Ordem de cálculo alterada com sucesso', life: 5000 });
            emit('closeDialog');
        })
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Não foi possível alterar a ordem de cálculo.', life: 5000 });
        });
};
...
<template>
    <Dialog v-model:visible="showDialogComputed" :style="{ width: '50%' }" :header="mode === 'create' ? 'Nova Ordem de Cálculo' : 'Alterar Ordem de Cálculo'" :modal="true" :closable="false" @show="showModal">
        <UWForm :schema="schema" :values="formData" ref="formOrdemCalculo" @doCancel="hideDialog" @doSubmit="salvarRegistro">
            <template #errors="{ errors }">
                <div class="col-12">
                    <div class="p-fluid formgrid grid">
                        ...
                        />
                        <UWInteger id="ordem" label="Ordem" required autofocus v-model="formData.ordem" :errors="errors.value?.ordem" classContainer="col-12 md:col-4" />
