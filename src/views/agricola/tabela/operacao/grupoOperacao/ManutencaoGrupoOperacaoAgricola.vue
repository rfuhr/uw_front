...
import { GrupoOperacaoAgricolaService as Service } from '@/service';
...
const criarRegistro = () => {
    Service.create(formData)
        .then(() => {
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Grupo de operação criado com sucesso', life: 5000 });
            emit('closeDialog');
        })
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Não foi possível criar o grupo de operação.', life: 5000 });
        });
};
...
const alterarRegistro = () => {
    ...
    Service.update(formData)
        .then(() => {
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Grupo de operação alterado com sucesso', life: 5000 });
            emit('closeDialog');
        })
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Não foi possível alterar o grupo de operação.', life: 5000 });
        });
};
...
<template>
    <Dialog v-model:visible="showDialogComputed" :style="{ width: '40%' }" :header="mode === 'create' ? 'Novo Grupo de Operação' : 'Alterar Grupo de Operação'" :modal="true" :closable="false" @show="showModal">
        <UWForm :schema="schema" :values="formData" ref="formSafra" @doCancel="hideDialog" @doSubmit="salvarRegistro">
