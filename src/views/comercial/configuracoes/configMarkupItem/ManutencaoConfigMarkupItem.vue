...
import { ConfigMarkupItemService as Service, ItemService } from '@/service';
import { parseISO } from 'date-fns';
import  ManutencaoConfigMarkupItemIndice from './ManutencaoConfigMarkupItemIndice.vue';
...
const criarRegistro = () => {
    Service.create(formData)
        .then(() => {
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Configuração de Mark Up do Item criada com sucesso', life: 5000 });
            emit('closeDialog');
        })
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Não foi possível criar a Configuração de Mark Up do Item.', life: 5000 });
        });
};
...
const alterarRegistro = () => {
    ...
    Service.update(formData)
        .then(() => {
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Configuração de Mark Up do Item alterada com sucesso', life: 5000 });
            emit('closeDialog');
        })
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Não foi possível alterar a Configuração de Mark Up do Item.', life: 5000 });
        });
};
...
const showModal = async () => {
    if (props.mode === 'create') {
        ...
        formData.configMarkupItemIndices = [];
    } else {
...
<template>
    <Dialog v-model:visible="showDialogComputed" :style="{ width: '90%' }" :header="mode === 'create' ? 'Nova Configuração de Mark Up do Item' : 'Alterar Configuração de Mark Up do Item'" :modal="true" :closable="false" @show="showModal">
        <UWForm :schema="schema" :values="formData" ref="formConfigMarkupItem" @doCancel="hideDialog" @doSubmit="salvarRegistro">
                ...
                <div class="col-12">
                    ...
                    <TabView class="col-12">
                        <TabPanel header="Índices" class="col-12">
                            <ManutencaoConfigMarkupItemIndice v-model="formData.configMarkupItemIndices" />
                        </TabPanel>
                    </TabView>
                </div>
            </template>
