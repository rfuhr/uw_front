...
const handleLimpar = () => {
    ...
    formData.value.grupoFinanceiroId = null;
    formData.value.fatoGeradorId = null;
    formData.value.numeroRecibo = null;
    formData.value.dataMovimentoInicial = parseISO('1900-01-01');
    formData.value.dataMovimentoFinal   = parseISO('2999-12-31');
    formData.value.nossoNumero = null;
    formData.value.numeroDocumento = null;
};

const selecionarRegistros = async () => {
    selecionando.value = true;
    const baixaRequest = {...formData.value};
    await TituloService.selecaoEstorno(baixaRequest)
        .then((response) => {
...
<template>
    <UWPageBase title="Estorno de Títulos" subtitle="Seleção de baixas para estorno">
                    ...
                    <div class="col-12">
                        <UWFieldSet title="Filtros de Seleção">
                            <div class="p-fluid formgrid grid">
                                ...
                                />
                                <UWInteger id="numeroRecibo" label="Número Recibo" v-model="formData.numeroRecibo" classContainer="col-12 md:col-3" />
