...
import { TituloService, TipoTituloService, EmpresaFilialService, DepartamentoService, CaracteristicaMovimentoFinanceiroService, 
         CarteiraFinanceiraService, GrupoFinanceiroService, FatoGeradorService } from '@/service';
...
const selecionarRegistros = async () => {
    ...
    const selecaoNegociacaoRequest = {...formData.value};
    await TituloService.selecaoNegociacao(selecaoNegociacaoRequest)
        .then((response) => {
            emit('clickSelecao', response, formData.value.parceiroLocalId, formData.value.tipoTituloId);
        })
        .catch(() => {
...
<template>
    <UWPageBase title="Negociação de Títulos" subtitle="Seleção de títulos para negociação">
        <UWForm :schema="createSchema()" :values="formData" ref="formSelecaoNegociacao" @doSubmit="selecionarRegistros" 
                :visibleSave="false" :visibleCancel="false"> 
            <template #buttons>
                <Button label="Cancelar" icon="pi pi-replay" class="p-button-danger" outlined @click="handleCancelar()" />
                    ...
                    <div class="col-12">
                        <UWFieldSet title="Filtros de Seleção">
                            <div class="p-fluid formgrid grid">
                                ...
                                />
                                <UWParceiroLocal ref="seletorParceiroLocal" id="parceiroLocal" required classContainer="col-12 md:col-3" v-model="formData.parceiroLocalId" 
                                    label="Parceiro" :erros="errors.value?.parceiroLocalId"/>
                                <UWSeletor
