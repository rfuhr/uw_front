...
const createSchema = () => {
    return yup.object().shape({
        ...
        carteiraFinanceiraDestinoId: yup.number().required('Carteira de Destino é obrigatório'),
        dataMovimentoInicial: yup.date().required('Data Movimento Inicial é obrigatório'),
        dataMovimentoFinal: yup.date().required('Data Movimento Final é obrigatório').min(yup.ref('dataMovimentoInicial'), 'Data Movimento Final deve ser maior que Data Movimento Inicial'),
        vencimentoInicial: yup.date().required('Vencimento Inicial é obrigatório'),
        vencimentoFinal: yup.date().required('Vencimento Final é obrigatório').min(yup.ref('vencimentoInicial'), 'Vencimento Final deve ser maior que Vencimento Inicial')
    });
};
...
const selecionarRegistros = async () => {
    ...
    const substituicaoRequest = { ...formData.value };
    await TituloService.selecaoSubstituicaoCarteira(substituicaoRequest)
        .then((response) => {
            response.forEach((item) => {
                if (!item.juros) item.juros = [];
                if (!item.descontos) item.descontos = [];
            });

            emit('clickSelecao', response, formData.value.carteiraFinanceiraDestinoId);
        })
        .catch(() => {
...
<template>
    <UWPageBase title="Substiuição de Carteira" subtitle="Seleção de títulos para substituição carteira">
        <UWForm :schema="createSchema()" :values="formData" ref="formSelecaoBaixa" @doSubmit="selecionarRegistros" :visibleSave="false" :visibleCancel="false">
            ...
            <template #errors="{ errors }">
                <div class="grid nested-grid">
                    <div class="col-12">
                        <UWFieldSet title="Carteira de Destino">
                            <div class="p-fluid formgrid grid">
                                <UWSeletor
                                    id="carteiraFinanceira"
                                    classContainer="col-12 md:col-12"
                                    v-model="formData.carteiraFinanceiraDestinoId"
                                    required
                                    optionLabel="nome"
                                    optionValue="id"
