...
const selecionarRegistros = async () => {
    ...
    selecaoRelatorioRequest.tipoRelatorioFinanceiro = 'ANALITICOADTODEVABERTO';
    RelatorioFinanceiroService.imprimirRelatorio(JSON.stringify(selecaoRelatorioRequest)).then((response) => {
        ...
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Erro ao gerar o Relatório Análitico Adiantamentos / Devoluções em Aberto.', life: 5000 });
            blocked.value = false;
        });
};
...
<template>
    ...
    <UWPageBase title="Adiantamento / Devoluções em Aberto" subtitle="Filtro para seleção de títulos de adiantamento / devoluções em aberto">
