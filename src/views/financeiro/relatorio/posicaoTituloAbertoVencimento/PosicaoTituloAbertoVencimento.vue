...
const selecionarRegistros = async () => {
    ...
    selecaoRelatorioRequest.tipoRelatorioFinanceiro = 'POSTITULOABERTOVENCIMENTO';
    RelatorioFinanceiroService.imprimirRelatorio(JSON.stringify(selecaoRelatorioRequest)).then((response) => {
        ...
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Erro ao gerar o Relatório de Posição de Título em Aberto por Vencimento.', life: 5000 });
            blocked.value = false;
        });
};
...
<template>
    ...
    <UWPageBase title="Posição de Títulos em Aberto por Vencimento">
