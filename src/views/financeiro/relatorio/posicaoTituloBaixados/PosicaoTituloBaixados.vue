...
import { TipoTituloService, EmpresaFilialService, DepartamentoService, CaracteristicaMovimentoFinanceiroService, 
         CarteiraFinanceiraService, RelatorioFinanceiroService, TipoOperacaoFinanceiraService } from '@/service';

const createSchema = () => {
    return yup.object().shape({
        empresaFilialId: yup.number().required('Filial é obrigatório'),
        competencia: yup.date().required('Competência é obrigatório'),
        dataMovimentoInicial: yup.date().required('Data de Movimento Inicial é obrigatório'),
        dataMovimentoFinal: yup.date()
            .required('Data de Movimento Final é obrigatório')
            .min(yup.ref('dataMovimentoInicial'), 'Data de Movimento Final deve ser maior que Data de Movimento Inicial')
    });
};
...
const handleLimpar = () => {
    ...
    formData.value.tipoOperacaoFinanceiraId = null;
    formData.value.dataMovimentoInicial = parseISO('1900-01-01');
    formData.value.dataMovimentoFinal = parseISO('2999-12-31');
};
...
const selecionarRegistros = async () => {
    ...
    selecaoRelatorioRequest.dataMovimentoInicial = formatToUTC(selecaoRelatorioRequest.dataMovimentoInicial);
    selecaoRelatorioRequest.dataMovimentoFinal = formatToUTC(selecaoRelatorioRequest.dataMovimentoFinal);
    selecaoRelatorioRequest.tipoRelatorioFinanceiro = 'POSTITULOBAIXADOS';
    RelatorioFinanceiroService.imprimirRelatorio(JSON.stringify(selecaoRelatorioRequest)).then((response) => {
        ...
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Erro ao gerar o Relatório de Posição de Título Baixados.', life: 5000 });
            blocked.value = false;
        });
};
...
<template>
    ...
    <UWPageBase title="Posição de Títulos Baixados" subtitle="Filtro para seleção de títulos baixados">
                        ...
                        <UWFieldSet title="Filtros de Seleção">
                            <div class="p-fluid formgrid grid">
                                ...
                                <UWSeletor
                                    ...
                                    :columnsFilters="[{ field: 'listaPosicaoTituloBaixados', value: true, matchMode: 'equal', tipoField: 'boolean', fieldFilter: 'listaPosicaoTituloBaixados' }]"
                                />
                                ...
                                />
                                <UWCalendar id="dataMovimentoInicial" label="Data de Movimento Inicial" required v-model="formData.dataMovimentoInicial" classContainer="col-12 md:col-3" :errors="errors.value?.vencimentoInicial" />
                                <UWCalendar id="dataMovimentoFinal" label="Data de Movimento Final" required v-model="formData.dataMovimentoFinal" classContainer="col-12 md:col-3" :errors="errors.value?.vencimentoFinal" />
                            </div>
                        </UWFieldSet>
                    </div>
                </div>
