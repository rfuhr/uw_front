...
const selecionarRegistros = async () => {
    ...
    selecaoRelatorioRequest.tipoRelatorioFinanceiro = 'SINTETICOGERAL';
    RelatorioFinanceiroService.imprimirRelatorio(JSON.stringify(selecaoRelatorioRequest)).then((response) => {
        ...
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Erro ao gerar o Relatório Sintético Geral.', life: 5000 });
            blocked.value = false;
        });
};
...
<template>
    ...
    <UWPageBase title="Sintético Geral" >
