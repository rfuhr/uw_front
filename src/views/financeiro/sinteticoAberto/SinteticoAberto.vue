...
const selecionarRegistros = async () => {
    ...
    selecaoRelatorioRequest.tipoRelatorioFinanceiro = 'SINTETICOABERTO';
    RelatorioFinanceiroService.imprimirRelatorio(JSON.stringify(selecaoRelatorioRequest)).then((response) => {
        ...
        .catch(() => {
            toast.add({ severity: 'error', summary: 'Falha', detail: 'Erro ao gerar o Relatório Sintético em Aberto.', life: 5000 });
            blocked.value = false;
        });
};
...
<template>
    ...
    <UWPageBase title="Sintético em Aberto" >
